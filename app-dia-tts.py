...
try:
    # Specific import for Dia model
    from dia.model import Dia
    # Keep torch and soundfile
except ImportError:
...
# Dia example uses 44100 Hz
DIA_SAMPLING_RATE = 44100

HOST = os.getenv("FLASK_HOST", "127.0.0.1")
# Use a different default port for the Dia app, e.g., 3005
PORT = int(os.getenv("FLASK_PORT_DIA", "3005"))
MAX_TEXT_LENGTH = int(os.getenv("MAX_TEXT_LENGTH", "1000")) # Keep max length configurable

# --- Flask App Setup ---
app = Flask(__name__)
# Configure logging specifically for the Dia app
...
model: Optional[Dia] = None # Type hint for Dia model
...
def load_model() -> None:
    ...
    try:
        ...
        # Dia might prefer float32 or handle dtypes internally, using float32 for consistency
        dtype = torch.float32
        ...
        # Load model specifically for Dia-TTS
        # Note: Dia's from_pretrained might handle dtype and device differently. Adjust if needed.
        model = Dia.from_pretrained(DIA_MODEL_ID)

        # Check if model has an eval method, common practice
...
def synthesize() -> Union[Tuple[Response, int], Response]:
    ...
    try:
        ...
        with torch.no_grad(): # Ensure gradients are not computed
            # Dia-TTS specific generation
            app.logger.debug("Generating audio with Dia model...")
            # Assuming model.generate takes text directly and returns the waveform tensor
            output_tensor = model.generate(text_to_speak)

            # Ensure the output is a tensor and move to CPU if needed
            ...
            waveform = output_tensor.to('cpu') # Move to CPU for numpy conversion
        ...
        # Use the specific sampling rate for Dia
        ...
        buffer = io.BytesIO()
        # Using WAV format for consistency, though MP3 was in the example
...
if __name__ == '__main__':
    try:
        ...
        app.logger.info(f"Model Data Type: {model.dtype if hasattr(model, 'dtype') else 'N/A'}") # Log dtype if available
